...
AIPro를 활용하여 사업건전성 대시보드를 고도화하는 4개 과제 각각에 대해, 개발자 미팅 전에 확인해야 할 기술적 문제를 정리한 자료입니다.
...
1. AIPro에서 사내 DB 직접 접속 가능 여부
2. 과제 4 데이터 수집 방식 — DB 직접 읽기(권장) vs 대시보드 읽기
3. AIPro 코드 실행 환경 범위 (프론트/백엔드/SQL)
4. AIPro 스케줄 자동 실행 지원 여부
5. AIPro와 사내 메일 시스템 연동 가능 여부
