...
description: 기업명을 입력하면 HTML 형식의 기업조사 분석 보고서를 자동 작성·문서허브 업로드하는 Claude 스킬
---
...
| 원칙 | 내용 |
|------|------|
| 수치 출처 명시 | 매출·임직원수 등 정량 데이터는 출처·기준연도를 각주 또는 셀 내 명시 |
| 불확실 정보 표시 | 비공개 수치는 "(추정)" 또는 "미공개"로 명기 — 임의 추정 금지 |
| 표 우선 | 임원 프로필·법인 현황·재무 추이는 반드시 표로 정리 |
| 섹션 생략 기준 | 근거 데이터 없으면 해당 섹션 전체 삭제 — 빈 섹션 금지 |
| 원문 우선 | 사용자가 붙여넣은 자료가 있으면 1차 소스로 사용, 웹 검색은 보완용 |
...
| 파일 | 설명 |
|------|------|
| `~/.claude/skills/corp-report/SKILL.md` | 5단계 실행 흐름 정의 |
| `~/.claude/skills/corp-report/references/html-template.md` | CSS + 섹션별 HTML 구조 템플릿 |
